...
public class Villagerbunker1Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (!dimensionCriteria)
						return false;
					if ((random.nextInt(1000000) + 1) <= 505) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("latex", "villager_bunker_1"));
							if (template == null)
								return false;
									...
									random);
							{
								...
								Villagerbunker1OnStructureInstanceGeneratedProcedure.executeProcedure($_dependencies);
							}
						}
					}
					return true;
			...
			event.getRegistry().register(feature.setRegistryName("villagerbunker_1"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("latex:villagerbunker_1"), configuredFeature);
		}
	}
	@SubscribeEvent
