...
public class OldTVBlock extends LatexModElements.ModElement {
	@ObjectHolder("latex:old_tv")
	public static final Block block = null;
	public OldTVBlock(LatexModElements instance) {
		super(instance, 131);
	}
	...
	public static class CustomBlock extends Block implements IWaterLoggable {
		...
		public CustomBlock() {
			super(Block.Properties.create(Material.ANVIL).sound(SoundType.METAL).hardnessAndResistance(2f, 17.5f).setLightLevel(s -> 0)
					.harvestLevel(1).harvestTool(ToolType.PICKAXE).setRequiresTool().notSolid().setOpaque((bs, br, bp) -> false));
			this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(WATERLOGGED, false));
			setRegistryName("old_tv");
		}
		...
		public void addInformation(ItemStack itemstack, IBlockReader world, List<ITextComponent> list, ITooltipFlag flag) {
			...
			list.add(new StringTextComponent("Requires Redstone to work"));
		}
			...
			if (world.getRedstonePowerFromNeighbors(new BlockPos(x, y, z)) > 0) {
				{
					...
					OldTVRedstoneOnProcedure.executeProcedure($_dependencies);
				}
			} else {
			}
		}
